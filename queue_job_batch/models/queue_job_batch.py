...
class QueueJobBatch(models.Model):
    ...
    job_ids = fields.One2many(
        "queue.job", inverse_name="job_batch_id", readonly=True
    )
    job_count = fields.Integer(compute="_compute_job_count")
    user_id = fields.Many2one(
        "res.users", required=True, readonly=True, track_visibility="onchange"
    )
    ...
    def _compute_job_count(self):
        for record in self:
            ...
            done_job_count = len(
                record.job_ids.filtered(lambda r: r.state == "done")
            )
